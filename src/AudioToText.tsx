...
const AudioToText: React.FC = () => {
  ...
  const connect = () => {
    ...
    const socket = io.connect("https://am-sst.onrender.com");
    socket.on("connect", () => {
  ...
  const disconnect = () => {
    ...
    // Removed setRecorder(undefined); as it's not defined or used in your component
    setIsRecording(false);
  };

  const clearTranscriptions = () => {
    setCurrentRecognition(""); // Clear the current recognition
    setRecognitionHistory([]); // Clear the recognition history
  };
      ...
      <Container className="py-5 text-center">
        <Container fluid className="py-5 bg-primary text-light text-center">
          <Container>
            ...
            <Button className="btn-warning" onClick={clearTranscriptions}>
              Clear
            </Button>
          </Container>
        </Container>
